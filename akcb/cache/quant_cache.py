...
from akcb.calculator import adjust_budgets, append_new_indices, build_unified_positions, calculate_heavy_hitter, dequantize_tensor, quantize_tensor
...
class QuantCache(Cache):
    ...
    def log_state(self, place, logging=False, logging_place=False):
        if not logging_place:
            return
        print(f"[JLE] QuantCache at {place}: {len(self)} layers")
        if not logging:
            return
