...
class WindowCache(Cache):
    """
    Cache for storing the original key-value pairs.
    """
    
    def __init__(self) -> None:
        self.layers: List[WindowLayer] = []
        ...
        self.gamma = 0.5
        self.mix_type = "window"  # "mix", "quant", "origin"
    ...
    def update(
        ...
        if layer_idx >= len(self.layers):
            self.layers.append(WindowLayer())
        if key_states is None or value_states is None:
            return self.layers[layer_idx].get_kv_cache()
    ...
    @classmethod
    def from_legacy_cache(cls, past_key_values: Optional[Tuple[Tuple[torch.FloatTensor]]] = None) -> "WindowCache":
        """Converts a cache in the legacy cache format into an equivalent `WindowCache`. Used for
        backward compatibility."""
        cache = cls()
    ...
    @classmethod
    def from_dynamic_cache(cls, past_key_values: Optional[DynamicCache] = None) -> "WindowCache":
        cache = cls()
        if past_key_values is not None:
    ...
    def batch_split(self, full_batch_size: int, split_size: int) -> List["WindowCache"]:
        """Split the current instance into a list of `WindowCache` by the batch size. This will be used by
        `_split_model_inputs()` in `generation.utils`"""
        out = []
        for i in range(0, full_batch_size, split_size):
            current_split = WindowLayer()
...
class WindowPrefillKVCompressor:
    def __init__(
        self,
    ...
    def evict_layer_kvcache(self, past_key_values: WindowCache, layer_idx: int, layer_budget: int):
        ...
        #     return past_key_values
        # B, H, _, D = past_key_values.origin_key_cache[layer_idx].shape
        ...
        # k_past_compress = past_key_values.origin_key_cache[layer_idx][:, :, :-self.window_size, :].gather(dim=2, index=indices)
        # v_past_compress = past_key_values.origin_value_cache[layer_idx][:, :, :-self.window_size, :].gather(dim=2, index=indices)
        o_len = past_key_values.get_seq_length(layer_idx)
        if o_len <= self.window_size:
        ...
        # past_key_values.origin_key_cache[layer_idx] = k_cur
        # past_key_values.origin_value_cache[layer_idx] = v_cur
...
class WindowDecodingKVCompressorLayerWise:
    def __init__(
        self,
        cache_config: ADCacheConfig,
    ):
