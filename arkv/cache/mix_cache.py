...
class MixLayer(CacheLayerMixin):
    ...
    def update(
        ...
        if self.origin_keys.numel() == 0:
            ...
            new_idx = torch.arange(0, new_T, device=key_states.device, dtype=torch.long)
    ...
    def crop(self, max_length: int) -> None:
        ...
        if key_states is None:
            return
        B, H, _, D = key_states.shape
        new_idx = torch.arange(0, max_length, device=key_states.device, dtype=torch.long)
        new_idx = new_idx.unsqueeze(0).unsqueeze(0).unsqueeze(-1).expand(B, H, -1, D)  # [B, H, new_T, D]
        self.set_layer_info(
    ...
    def batch_repeat_interleave(self, repeats: int) -> None:
        ...
        if key_states.numel() > 0:
            ...
            new_idx = torch.arange(0, L, device=key_states.device, dtype=torch.long)
...
class MixCache(Cache):
    
    def __init__(self) -> None:
        self.layers: List[MixLayer] = []
        self.num_layers: int = 0

        self.oq_scores:  List[torch.Tensor] = []
        self.quant_ratios: List[torch.Tensor] = []
        self.evict_scores: List[torch.Tensor] = []
        self.origin_budget: List[int] = []
        self.gamma = 0.5
        self.mix_type = "mix"  # "mix", "quant", "origin"
        self.o_dtype = torch.float16
        
    def get_quant_ratio(self, layer_idx: int) -> float:
        """Get the quantization ratio for a given layer."""
        if layer_idx < len(self.quant_ratios):
            return self.quant_ratios[layer_idx]
        else:
            return 0.0
    ...
    @classmethod
    def from_legacy_cache(cls, past_key_values: Optional[Tuple[Tuple[torch.FloatTensor]]] = None) -> "MixCache":
        """Converts a cache in the legacy cache format into an equivalent `MixCache`. Used for
        backward compatibility."""
        cache = cls()
    ...
    @classmethod
    def from_dynamic_cache(cls, past_key_values: Optional[DynamicCache] = None) -> "MixCache":
        cache = cls()
        if past_key_values is not None:
            cache.num_layers = len(past_key_values.layers)
            ...
            if cache.num_layers > 0:
                cache.origin_budget = [0 for _ in range(cache.num_layers)]
        return cache
...
class MixPrefillKVCompressor:
    def __init__(
        ...
        # self.print_oq_ratio = cache_config.print_oq_ratio
    ...
    def __call__(self, past_key_values, seq_len):
        ...
        # OQ scores per layer. Use the max to normalize into [0, 1].
        oq_scores = past_key_values.oq_scores
        max_oq = max(oq_scores) if len(oq_scores) > 0 else 0.0

        # Compute per-layer origin budgets for the PAST part (exclude window).
        # ratio = oq / max_oq; higher score => more FP16 in the past segment.
        try:
            if max_oq > 0:
                origin_budgets = [int(round((s / max_oq) * available_size)) for s in oq_scores]
    ...
    def evict_layer_kvcache(self, past_key_values: MixCache, layer_idx: int, layer_budget: int):
        ...
        indices = hh_score.topk(layer_budget, dim=-1).indices.long()
        ...
        new_T = key_states.shape[2]
        o_idx = torch.arange(0, new_T, dtype=torch.long)
        o_idx = o_idx.unsqueeze(0).unsqueeze(0).unsqueeze(-1).expand(B, H, -1, D)  # [B, H, T, D]
...
class MixDecodingKVCompressorLayerWise:
    def __init__(
        self,
        cache_config: ADCacheConfig,
    ):
    ...
    def __call__(self, past_key_values: "MixCache", attn_score: torch.Tensor, layer_idx: int):
        ...
        if past_len == 0:
            ...
            full_idx = torch.arange(T, dtype=torch.long)
        ...
        # print(f"layer {layer_idx} evicting: o_len={o_len}, past_len={past_len}, keep_size={keep_size}")
        # print(f"B={B}, Hkv={Hkv}, Gkv={Gkv}, T={T}, D={D}")
        # print(f"attn_score: {attn_score.shape}")
        # print(f"hh_scores: {hh_scores.shape}")
        # print(f"hh_scores min/max: {hh_scores.min().item() if hh_scores.numel()>0 else None}/{hh_scores.max().item() if hh_scores.numel()>0 else None}")
                
        # 1) Calculate indices by hh_score and origin_budget
        origin_budget = past_key_values.origin_budget[layer_idx] - self.window_size
        if origin_budget > keep_size:
            origin_budget = keep_size
        if origin_budget < 0:
            origin_budget = 0
        quant_budget = past_len - origin_budget
        
        keep_indices = hh_scores.topk(keep_size, dim=-1).indices.long()  # [B, Hkv, k]
        o_indices = hh_scores.topk(origin_budget, dim=-1).indices.long()  # [B, Hkv, o]
        q_indices = heads_diff(keep_indices, o_indices)
        w_indices = torch.arange(past_len, o_len, device=device, dtype=torch.long)  # [window_size]
        w_indices = w_indices.unsqueeze(0).unsqueeze(0).expand(B, Hkv, -1)  # [B, Hkv, window_size]
        # print(f"o_indices: {o_indices.shape}, q_indices: {q_indices.shape}, w_indices: {w_indices.shape}")
        ...
        # 5) Update MixCache with new origin + quant
        past_key_values.set_layer_info(
            layer_idx,
            o_k=o_k, o_v=o_v,
            q_k=q_k, q_v=q_v,
